...
- (void)autoParse:(NSObject **)object fromJSONDict:(NSDictionary *)dict {
    ...
    for (NSString *key in properties) {
        ...
        NSString *type = [properties objectForKey:key];
        SEL setter = NSSelectorFromString([NSString stringWithFormat:@"set%@%@:", [[key uppercaseString] substringToIndex:1], [key substringFromIndex:1]]);
        ...
        switch ([type characterAtIndex:0]) {
            case '@':   // object
                if ([[type componentsSeparatedByString:@"\""] count] > 1) {
                    ...
                    [*object performSelector:setter withObject:[dict objectForKey:key]];
                }
                break;
            case 'c':   // bool
                break;
            ...
            case 'i':   // int
                if ([dict objectForKey:key]) {
                    i = [*object intForKey:key inDict:dict];
